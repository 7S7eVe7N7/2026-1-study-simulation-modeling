...
function vaccination_update!(sim, m, num_vacc)
    push!(m.ta, ConcurrentSim.now(sim))
    new_S = m.Sa[end] - num_vacc
    new_R = m.Ra[end] + num_vacc
    push!(m.Sa, new_S)
    push!(m.Ia, m.Ia[end])
    push!(m.Ra, new_R)
    count = 0
    for ind in m.allIndividuals
        if ind.status == :S && count < num_vacc
            ind.status = :R
            count += 1
        end
    end
...
@resumable function vaccinate(env, m, time, fraction)
    ...
    num_vacc = round(Int, fraction * m.Sa[end])
    vaccination_update!(env, m, num_vacc)
end
